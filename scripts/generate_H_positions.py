...
s_orig = Structure.from_file('POSCAR_NH4Fe2F6.vasp', sort=True)

oxi = OxidationStateDecorationTransformation({"N":-3, "O":-3, "Fe":2.5, "F":-1, "H":1})
...
'''
vit = VoronoiInsertionTransformation('H1+', n_h_per_N, bond_lengths={'N3-': 1.05}, midpoints=False, interior_only=False)
...
for j in range(len(n_indices)):
    ...
    if j == len(n_indices)-1:
        s.to(filename='POSCAR_NH4Fe2F6_with_H.vasp',fmt='poscar')
